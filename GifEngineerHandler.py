import os
import tkinter as tkt
from tkinter import filedialog
import imageio
import shutil
import cv2
from datetime import datetime
from PIL import Image, ImageTk, ImageSequence
from moviepy.editor import *
import base64
from io import BytesIO
import time


class GifEngineerHandler:
    def __init__(self):
        # For Images
        self.imagesList = []
        self.imageEntns = ['.JPG', '.PNG', '.JPEG', '.TIFF']
        self.imageSrcDir = ''
        self.trgtDir = ''
        self.tempDirName = '_temp'
        self.postFix = datetime.now()
        self.imageSrcDirTitle = 'Select Images Directory'

        # For Video
        self.videoSrcDir = ''
        self.videoSrcDirTitle = 'Select Video Directory'

        # For GIF
        self.gifSrcDirTitle = 'Select Gif Directory'
        self.gifEntns = ['.GIF']
        self.gifSrcDir = ''
        self.parentFrame = ''
        self.label = ''
        self.gifCanvas = ''
        self.gifSeq = ''
        self.gifImg = ''
        self.gifTrgtDirTitle = 'Save GIF @ Directory'
        self.gifBase64 = ''''''

    # create a frame to diaplay stuff
    def createFrame(self, window):
        frame = tkt.Frame(window, bg='#b5b5b5')  # frame creation
        frame.place(relwidth=0.8, relheight=0.8, relx=0.1, rely=0.1)
        return frame

    # select the Images directory path
    def loadImgSrcDir(self, window):
        self.clearGarbage()
        self.imageSrcDir = filedialog.askdirectory(title=self.imageSrcDirTitle)

        if self.imageSrcDir:
            self.gifFromImages(self.imageSrcDir, self.createFrame(window))

    # select the Video directory path
    def loadVideoSrcDir(self, window):
        self.clearGarbage()
        self.videoSrcDir = filedialog.askopenfilename(filetypes=[("MP4 files", "*.mp4"), ("MPEG files", "*.mpeg"), ("AVI files", "*.avi"), ("WMV files", "*.wmv"), ("MOV files", "*.mov"), ("FLV files", "*.flv"), ("WEBM files", "*.webm")],
                                                      title=self.videoSrcDirTitle)

        if self.videoSrcDir:
            self.gifFromVideo(self.videoSrcDir, self.createFrame(window))

    # select the Video directory path
    def loadGIFSrcDir(self, window):
        self.clearGarbage()
        self.gifSrcDir = filedialog.askdirectory(title=self.gifSrcDirTitle)

        if self.gifSrcDir:
            self.gifFromGifs(self.gifSrcDir, self.createFrame(window))

    # select the target directory path
    def handleSaveCancel(self, action, gifPath, frame):
        if action == 'save':
            gifTrgtDir = filedialog.askdirectory(
                initialdir=self.imageSrcDir, title=self.gifTrgtDirTitle)

            if gifTrgtDir:
                shutil.copy(os.path.normpath(gifPath),
                            os.path.normpath(gifTrgtDir))
                frame.destroy()
                self.clearGarbage()

        if action == 'cancel':
            os.remove(gifPath)
            frame.destroy()
            self.clearGarbage()

    # clear garbage
    def clearGarbage(self):
        if self.trgtDir:
            shutil.rmtree(self.trgtDir)
        self.imageSrcDir = ''
        self.videoSrcDir = ''
        self.gifSrcDir = ''
        self.trgtDir = ''
        self.imagesList = []

    # form a Gif using images
    def gifFromImages(self, srcDirPath, frame):
        self.trgtDir = os.path.join(self.imageSrcDir, self.tempDirName)
        if not os.path.isdir(self.trgtDir):
            print('Temporary directory created.')
            os.mkdir(self.trgtDir)
        gifFilename = 'gifFromImages' + \
            self.postFix.strftime('%m%d%Y_%H%M%S') + '.gif'

        print('Copying files into temporary directory..')
        # copy the images to a temp file for safety
        for filename in os.listdir(srcDirPath):
            source = srcDirPath + '/' + filename
            target = self.trgtDir + '/' + filename
            if filename is not None and os.path.isfile(source):
                fName, fExt = os.path.splitext(filename)
                if fExt.upper() in self.imageEntns:
                    shutil.copyfile(source, target)
                    self.imagesList.append(source)

        # create the gif and save to the destination
        try:
            print('Creating the GIF in temporary directory..')
            with imageio.get_writer(self.trgtDir + '/' + gifFilename, mode='I') as writer:
                for filePath in self.imagesList:
                    image = imageio.imread(filePath)
                    writer.append_data(image)
            print('Creation of Gif completed.')
        except:
            print('Error while creating the GIF.')

        self.saveCancelBtns(os.path.join(
            self.trgtDir, gifFilename), frame)

    # form a Gif using Video
    def gifFromVideo(self, vidSrcPath, frame):
        tempDirPath = os.path.split(vidSrcPath)[0]
        self.trgtDir = os.path.join(tempDirPath, self.tempDirName)
        if not os.path.isdir(self.trgtDir):
            print('Temporary directory created.')
            os.mkdir(self.trgtDir)
        gifFilename = 'gifFromVideo' + \
            self.postFix.strftime('%m%d%Y_%H%M%S') + '.gif'

        # using MoviePy to convert video to gif
        clip = (VideoFileClip(vidSrcPath))
        clip.write_gif(os.path.join(self.trgtDir, gifFilename))

        self.saveCancelBtns(os.path.join(
            self.trgtDir, gifFilename), frame)

    # Save and Cancel buttons
    def saveCancelBtns(self, gifPath, frame):
        self.renderGif(gifPath, frame)
        gifSaveBtnText = 'Save GIF @'
        gifCancelBtnText = 'Cancel/Delete Saved GIF'
        # Save Btn
        saveBtn = tkt.Button(frame, text=gifSaveBtnText, bg='#454545',
                             borderwidth=0, command=lambda: self.handleSaveCancel('save', gifPath, frame))  # lambda: frame.grid_forget())
        saveBtn.pack()
        saveBtn.place(bordermode='outside', x=0.1, y=0.1)

        # Cancel Btn
        cancelBtn = tkt.Button(frame, text=gifCancelBtnText, bg='#454545',
                               borderwidth=0, command=lambda: self.handleSaveCancel('cancel', gifPath, frame))
        cancelBtn.pack()
        cancelBtn.place(bordermode='outside', x=380, y=0.1)

    # Render the created Gif
    def renderGif(self, gifPath, gifFrame):
        self.parentFrame = gifFrame
        self.gifCanvas = tkt.Canvas(self.parentFrame, width=400, height=200)
        self.gifCanvas.pack()
        self.gifCanvas.place(bordermode='outside', x=90, y=50)
        self.gifSeq = [ImageTk.PhotoImage(img.resize((400, 250), Image.ANTIALIAS))
                       for img in ImageSequence.Iterator(Image.open(gifPath))]
        self.gifImg = self.gifCanvas.create_image(
            202, 77, image=self.gifSeq[0])
        self.animationSequence(1)

    def animationSequence(self, counter):
        self.gifCanvas.itemconfig(
            self.gifImg, image=self.gifSeq[counter])
        self.parentFrame.after(20, lambda: self.animationSequence(
            (counter+1) % len(self.gifSeq)))

    # Clean up after window Close
    def windowClose(self, window):
        print('Closing the Application.')
        if self.trgtDir:
            shutil.rmtree(self.trgtDir)
        window.destroy()

    # Iterate through the Gif Frame and return the frame
    def gifIterateFrames(self, image):
        try:
            i = 0
            while 1:
                image.seek(i)
                imageFrame = image.copy()
                if i == 0:
                    imagePalette = imageFrame.getpalette()
                else:
                    imageFrame.putpalette(imagePalette)
                yield imageFrame
                i += 1
        except EOFError:
            pass

    # Render the Loading Gif
    def gifFromGifs(self, gifSrcPath, frameCanvas):
        self.trgtDir = os.path.join(gifSrcPath, self.tempDirName)
        if not os.path.isdir(self.trgtDir):
            print('Temporary directory created.')
            os.mkdir(self.trgtDir)
        gifFilename = 'gifFromImages' + \
            self.postFix.strftime('%m%d%Y_%H%M%S') + '.gif'

        print('Copying files into temporary directory..')
        fileCount = 0
        # copy the images to a temp file for safety
        for filename in os.listdir(gifSrcPath):
            source = gifSrcPath + '/' + filename
            if filename is not None and os.path.isfile(source):
                fName, fExt = os.path.splitext(filename)
                if fExt.upper() in self.gifEntns:
                    image = Image.open(source)
                    for i, frame in enumerate(self.gifIterateFrames(image)):
                        imgFileName = self.trgtDir + '/' + 'frame%d.jpg' % fileCount
                        frame.convert('RGB').save(imgFileName, **frame.info)
                        self.imagesList.append(imgFileName)
                        fileCount += 1

        # create the gif and save to the destination
        try:
            print('Creating the GIF in temporary directory..')
            with imageio.get_writer(self.trgtDir + '/' + gifFilename, mode='I') as writer:
                for filePath in self.imagesList:
                    image = imageio.imread(filePath)
                    writer.append_data(image)
            print('Creation of Gif completed.')
        except:
            print('Error while creating the GIF.')

        self.saveCancelBtns(os.path.join(
            self.trgtDir, gifFilename), frameCanvas)
